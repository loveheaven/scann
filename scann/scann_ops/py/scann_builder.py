...
　　　　     *args表示任何多个无名参数，它本质是一个tuple；
        ...
        def inner(self, *args, **kwargs):
            ...
            # __code__.co_varnames将函数局部变量以元组的形式返回。
...
class ScannBuilder(object):
    ...
    def score_ah(
        ...
        if n_dims % dimensions_per_block == 0:
          # 格式化的字符串文字以“f”为前缀，类似于str.format（）接受的格式字符串。
          # 它们包含由花括号包围的替换字段。替换字段是表达式，在运行时进行评估，
          # 然后使用format（）协议进行格式化：
            proj_config = f"""
    ...
    """
        ...
        if tree_params is not None:
            ...
            # 这里的tree已经变成_factory_decorator.<locals>.func_taker.<locals>.inner
            # of <scann.scann_ops.py.scann_builder.ScannBuilder object
