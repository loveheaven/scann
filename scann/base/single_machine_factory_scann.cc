...
namespace research_scann
{
  namespace
  {
        ...
        const GenericSearchParameters &params, SingleMachineFactoryOptions *opts)
    {
      ...
      {
        // 这个宏的意思是失败就返回，成功就获得partitioner值，继续往下走。
      ...
      auto build_sq_leaf_lambda =
        ...
        auto searcher_or_error = ScalarQuantizedBruteForceSearcher::
            CreateFromQuantizedDatasetAndInverseMultipliers(
                ...
                params.pre_reordering_num_neighbors, params.pre_reordering_epsilon);
        if (!searcher_or_error.ok())
          return searcher_or_error.status();
    ...
    /*
     SingleMachineFactoryScann ran in ->
SingleMachineFactoryUntypedScann ran in ->
SingleMachineFactoryUntypedImpl ran in ->
SingleMachineFactoryImpl ran in ->
SingleMachineFactoryLeafSearcher ran in ->
TreeXHybridFactory ran in ->
TreeAhHybridResidualFactory ran in
*/
    template <typename T>
